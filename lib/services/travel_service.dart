...
class TravelService {
  ...
  TravelService(this._dio, {this.cancelToken});
  ...
  Future<TravelsModel?> historyUser() async {
    const storage = FlutterSecureStorage();
    final token = await storage.read(key: "token");
    try {
      final response = await _dio.get(
        '${AppConfig.instance.apiHost}historyUser',
        // 'http://172.16.90.115:8091/api/getAllConnectedParamedics',
        cancelToken: cancelToken,
        options: Options(
          contentType: "application/json",
      ...
      log('[TravelService -> historyUser] error: $e');
      return null;
    }
  }

  Future<TravelsModel?> historyDriver() async {
    const storage = FlutterSecureStorage();
    final token = await storage.read(key: "token");
    try {
      final response = await _dio.get(
        '${AppConfig.instance.apiHost}historyDriver',
        // 'http://172.16.90.115:8091/api/getAllConnectedParamedics',
        cancelToken: cancelToken,
        options: Options(
          contentType: "application/json",
      ...
      log('[TravelService -> historyDriver] error: $e');
      return null;
    }
  }
}