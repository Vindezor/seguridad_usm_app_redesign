...
class AdministrateDriversController extends ChangeNotifier{

  final UserService driverService = UserService(Dio());
  List<User>? drivers;

  AdministrateDriversController();
  ...
  deleteDriver(context, id){
    showAlertOptions(
      ...
      acceptOnPressed: () async {
        Navigator.of(context).pop();
        globalLoading(context);
        final response = await driverService.deleteDriver(id);
        if(response != null){
          if(response.status == "SUCCESS"){
            Navigator.of(context).pop();
            showAlertOptions(
  ...
  void dispose() {
    ...
    super.dispose();
    log("[AdministrateDriversController] disposed");
  }
}

final administrateDriversController = ChangeNotifierProvider.autoDispose((_) => AdministrateDriversController());