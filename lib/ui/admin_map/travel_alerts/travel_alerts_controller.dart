...
class TravelAlertsController extends ChangeNotifier{

  TravelService travelService = TravelService(Dio());
  
  List<Alert>? alerts;

  TravelAlertsController(){
    log("[TravelAlertsController] init");
  }

  getAllAlerts(context, args) async {
    globalLoading(context);
    final idTravel = args["id_travel"];
    final response = await travelService.getAllAlertsByIdTravel(idTravel);
    if(response != null){
  ...
  // final AlertService userService = AlertService(Dio(BaseOptions(connectTimeout: const Duration(seconds: 10),)));
  ...
  //   globalLoading(context);
  //   final TravelAlertsModel? response = await userService.login(document: documentController.value.text, password: passwordController.value.text);
  //   if(response != null){
  //     if(response.status == "SUCCESS"){
  ...
  //           await storage.write(key: "type_user", value: response.data?.user.typeAlert.typeAlert);
  //           await storage.write(key: "id_type_user", value: response.data?.user.typeAlert.id.toString());
  //           Navigator.of(context).pop();
  ...
  void dispose() {
    ...
    log("[TravelAlertsController] disposed");
  }
}

final travelAlertsController = ChangeNotifierProvider.autoDispose((_) => TravelAlertsController());