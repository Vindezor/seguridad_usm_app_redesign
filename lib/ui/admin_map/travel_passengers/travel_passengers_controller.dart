...
class TravelPassengersController extends ChangeNotifier{

  TravelService travelService = TravelService(Dio());
  
  List<User>? passegers;

  TravelPassengersController(){
    log("[TravelPassengersController] init");
  }

  getAllPassengers(context, args) async {
    globalLoading(context);
    final idTravel = args["id_travel"];
    final response = await travelService.getAllPassengersByIdTravel(idTravel);
    if(response != null){
      if(response.status == "SUCCESS"){
        passegers = response.data;
        notifyListeners();
  ...
  //   globalLoading(context);
  //   final TravelPassengersModel? response = await userService.login(document: documentController.value.text, password: passwordController.value.text);
  //   if(response != null){
  //     if(response.status == "SUCCESS"){
  ...
  void dispose() {
    ...
    log("[TravelPassengersController] disposed");
  }
}

final travelPassengersController = ChangeNotifierProvider.autoDispose((_) => TravelPassengersController());